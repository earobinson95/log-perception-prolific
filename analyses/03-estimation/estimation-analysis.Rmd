...
write.csv(estimation_simulated_data, file = here("analyses/03-estimation/data/estimation-simulated-data.csv"), row.names = F, na = "")
write.csv(estimation_questions, file = here("analyses/03-estimation/data/estimation-questions.csv"), row.names = F, na = "")
write.csv(estimation_scenario_text, file = here("analyses/03-estimation/data/estimation-scenario-text.csv"), row.names = F, na = "")
write.csv(estimation_parameters, file = here("analyses/03-estimation/data/estimation-parameters.csv"), row.names = F, na = "")
```
...
q0_text_orig <- estimation_model_data %>% 
  filter(q_id == "Q0") %>%
  select(scale, response) %>%
  as_tibble() %>%
  unnest_tokens(word, response)  %>%
  mutate(stem_word = text_tokens(word, stemmer = stem_hunspell)) %>%
  mutate(stem_word = ifelse(stem_word == "crease", "increase", stem_word))

q0_text_clean1 <- q0_text_orig %>%
  anti_join(stop_words)

nums <- q0_text_clean1 %>% filter(str_detect(word, "^[0-9]")) %>% select(word) %>% unique()
q0_text_clean2 <- q0_text_clean1 %>%
  anti_join(nums, by = "word") %>%
  unnest(stem_word)

q0_text_summary <- q0_text_clean2 %>%
  group_by(scale, stem_word) %>%
  summarize(count = n()) %>%
  arrange(scale, -count)
# write.csv(q0_text_summary, file = here("data/03-estimation/q0-text-summary.csv"))
```
...
write.csv(qe1_pop_est, file = here("analyses/03-estimation/data/first-level-population-estimates.csv"), row.names = F, na = "")
