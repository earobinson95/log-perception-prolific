...
completed_sections <- dbReadTable(db_con,"completed_sections") %>% filter(section_complete == "lineups")
dbDisconnect(db_con)
...
lineup_model_data <- lineup_data_orig %>%
  filter(recruitment != "I am the researcher",
         prolific_id != "Too Many Users Sorry, but this application has exceeded its quota of concurrent users. Please try again later.") %>%
  ...
  mutate(participant_id = as.factor(digest(paste(nick_name, study_starttime, prolific_id, sep = "-"), "md5")), .before = curvature) %>%
  ...
  full_join(completed_sections, by = c("nick_name", "ip_address", "prolific_id", "study_starttime")) %>%
...
```{r}
glmm_mod <- glmer(response ~ curvature*scale + 
                            (1 | participant_id) +
                            (1 | dataset_id),
                  data = lineup_model_data,
                  family = binomial(link = "logit"))
car::Anova(glmm_mod) %>% broom::tidy() %>% knitr::kable(digits = 3, caption = "Lineups GLMM ANOVA")
```
