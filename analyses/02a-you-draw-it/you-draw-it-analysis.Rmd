...
# you draw it data
db_con <- dbConnect(dbDriver("SQLite"), dbname = here("perception-of-statistical-graphics/databases/test-run/02_you_draw_it_db.db"))
...
youdrawit_model_data %>%
  ggplot(aes(x = x)) +
  geom_line(aes(y = ynls, group = interaction(plot_id, scale)), color = "black", alpha = 0.5) +
  geom_line(aes(y = ydrawn, group = interaction(plot_id, scale), color = scale), alpha = 0.5) +
...
youdrawit_model_data %>%
  ...
  geom_line(aes(group = interaction(plot_id, scale), color = scale), alpha = 0.5) +
